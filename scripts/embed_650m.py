"""One-click ESM-2 650M embedding of the viral + bacterial datasets (GPU box).

Companion to scripts/embed_3b.py — identical pipeline, smaller backbone. Computes
windowed, mean-pooled per-protein embeddings with the 650M backbone
(`facebook/esm2_t33_650M_UR50D`) for every protein in the viral and bacterial
record dumps, and writes them to the repo's shared embedding cache:

    data/embeddings_cache/esm2_650M_mean_layer-last_win.npz

That is exactly the cache `va-train --model 650M --pooling mean` and the serving
loader look for, so once this finishes you can train/serve the 650M viral,
bacterial, OR unified models with no further embedding work. The cache is keyed by
accession and shared across domains, so BOTH datasets go into the one file — this
matters: a bacterial-only run would overwrite and clobber any existing viral 650M
vectors, so we always embed both to keep the cache complete.
...
Why this config:
  ...
  * It does NOT build the per-residue attention cache (that would be hundreds of
    GB at 337K proteins). Train MF with `--pooling mean` to use this cache.
...
Robustness:
  ...
  * First run downloads the 650M weights (~2.5 GB) from Hugging Face.

Hardware: 650M is small — comfortable on an L4 (24 GB) or any modern GPU under
fp16 autocast with the default token-budget batching. The full bacterial set
(~337K proteins) is the bulk of the work; viral (~17K) is quick. Falls back to CPU
with a loud warning (don't — it would take days).

Run it:  python scripts/embed_650m.py
"""
...
MODEL_KEY = "650M"        # facebook/esm2_t33_650M_UR50D, dim 1280
POOLING = "mean"
WINDOW = True             # windowed pooling -> esm2_650M_mean_layer-last_win.npz
...
def _check_deps() -> None:
    ...
    if missing:
        print(
            ...
            "or, for a CUDA-enabled PyTorch on Linux:\n"
...
def main() -> None:
    print("=" * 70)
    print("ESM-2 650M embedding (windowed mean pooling) — viral + bacterial")
    print("=" * 70)
    _check_deps()
    _report_device()
    ...
    print("Next: copy the .npz back into data/embeddings_cache/, then compare:")
    print("  va-train --domain bacterial --model 650M --pooling mean --no-save "
          "--records data/bacterial_reviewed.jsonl")
    print("  va-train --domain bacterial --model 3B   --pooling mean --no-save "
          "--records data/bacterial_reviewed.jsonl")
    print("=" * 70)
